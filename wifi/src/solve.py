...
class Client:
    ...
    def send_dhcp_request(self):
        ...
        discover = Ether(dst='ff:ff:ff:ff:ff:ff', src=self.mac, type=0x0800) \
                   / IP(src='0.0.0.0', dst='255.255.255.255') \
                   / UDP(dport=67,sport=68) \
                   / BOOTP(op=1, chaddr=smac) \
                   / DHCP(options=[('message-type','discover'), ('end')])

        packet =  self.get_radiotap_header() \
                    / self.encrypt(discover, key_idx=0)
        printd("send dhcp  request")
    ...
    def ping(self, ip, dst_ether):
        server_ip = "1.2.3.4"
        ping_packet = Ether(src=self.mac, dst=dst_ether) \
           / IP(dst=ip, src=self.ip) \
           / ICMP(type=8, seq=0x1337, id=0x1337) \
           / b"HELLO"
        self.do_send(ping_packet)

    def send_boat_cmds(self, boat_ip, dst_ether):
        #send udp
        cmd = Ether(src=self.mac, dst=dst_ether) \
            / IP(dst=boat_ip, src=self.ip) \
            / UDP(dport=2422, sport=9999) \
            / "info"
        self.do_send(cmd)
        ...
        if self.cur_ssid not in self.finished:
            self.finished += [self.cur_ssid]
    ...
    def recv_pkt(self, packet):
        ...
        if Dot11Beacon in packet:
            if packet[Dot11Elt].ID == 0:
                ssid = packet[Dot11Elt].info.decode('ascii')
                if ssid not in self.finished and self.cur_ssid != ssid: #self.cur_ssid in self.finished and self.cur_ssid != ssid and ssid not int :
                    printd("connect to new ssid " + ssid)
                    #printd(packet.show(dump=1))
                    self.target_bssid = packet.addr3
                    self.cur_ssid = ssid
                    self.connected = 0
                    self.eapol_state = 0
                    self.PSK = gen_psk(ssid)
        ...
        if Dot11CCMP in packet and self.connected > 3:
            ...
            if decrypted:
                if DHCP in decrypted:
                    #okay got DHCP offer...
                    req_type = next(opt[1] for opt in decrypted[DHCP].options if isinstance(opt, tuple) and opt[0] == 'message-type')
                    if req_type == 2:
                        printd("offer...")
                        #got offer. now we would send request
                        # but for CLI w ecan skip it
                        #send a pin g
                        my_ip = decrypted[BOOTP].yiaddr
                        self.ip = my_ip
                        server_ip = decrypted[BOOTP].siaddr
                        self.router_ip = server_ip
                        self.subnet = '.'.join(self.ip.split('.')[:3])


                        for i in range(3, 256):
                            self.ping(self.subnet + ".%d"%i, decrypted[Ether].src)

                elif UDP in decrypted:
                    printd("udp...")
                    printd(decrypted[UDP].load)
                elif ICMP in decrypted:
                    if decrypted[ICMP].type == 0:
                        printd("got icmp reply")
                        #printd(decrypted.show(dump=1))
                        self.send_boat_cmds(decrypted[IP].src, decrypted[Ether].src)

                        #printd("try udp send")
                        #send udp
                        cmd = Ether(src=self.mac, dst=decrypted[Ether].src) \
                            / IP(dst=decrypted[IP].src, src=self.ip) \
                            / UDP(dport=2422, sport=9999) \
                            / "info"
                        #self.do_send(cmd)
                    elif decrypted[ICMP].type == 3:
                        if decrypted[ICMP].code != 1:
                            printd("failed to reach")
                            printd(decrypted.show(dump=1))
                    #printd(decrypted.show(dump=1))
                else:
...
if __name__ == "__main__":
    ...
    client = Client("defcon2023", "defcon2023", mac="66:00:00:00:01:00", mode="stdio")
    client.run()
    printd("done")
